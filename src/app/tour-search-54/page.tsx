...
const StarRating: React.FC<StarRatingProps> = ({ rating, reviewCount }) => {
  const renderStar = (index: number) => {
    ...
    const filled = rating >= starValue ? 1 : Math.max(0, rating - index)
    ...
    return (
      <div key={index} className="relative inline-block w-4 h-4">
        ...
        <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{ clipPath: `inset(0 ${100 - filled * 100}% 0 0)` }}>
          <path
            d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
            fill="#FBBF24"
          />
...
const QuotedText: React.FC<QuotedTextProps> = ({ children }) => {
  return (
    <span className="relative">
      ...
      {children}
      <svg className="inline-block w-3 h-3 align-text-top" viewBox="0 0 24 24" fill="none">
        <path d="M19.417 6.679C20.447 7.773 21 9 21 10.989c0 3.5-2.457 6.637-6.03 8.188l-.893-1.378c3.335-1.804 3.987-4.145 4.247-5.621-.537.278-1.24.375-1.929.311-1.804-.167-3.226-1.648-3.226-3.489a3.5 3.5 0 013.5-3.5c1.073 0 2.099.49 2.748 1.179zm-10 0C10.447 7.773 11 9 11 10.989c0 3.5-2.457 6.637-6.03 8.188L4.077 17.8c3.335-1.804 3.987-4.145 4.247-5.621-.537.278-1.24.375-1.929.311C4.591 12.323 3.169 10.842 3.169 9.001a3.5 3.5 0 013.5-3.5c1.073 0 2.099.49 2.748 1.179z" fill="white" opacity="0.9"/>
      </svg>
    </span>
  )
}
...
export default function TourSearch32() {
  ...
  // Auto slide state for Japan 2 (Kyoto) card
  const [currentJapan2ImageIndex, setCurrentJapan2ImageIndex] = useState(0)
  const japan2Images = [
    "https://drive.google.com/uc?export=view&id=1HAl4iXIeimYtUgs2_LtNoiqpHfc3bQgM",
    "https://drive.google.com/uc?export=view&id=1B5k-tXgBL8mrvIVqv_nK9e9goluppT4e",
    "https://drive.google.com/uc?export=view&id=1EJG6tH0nwDi8eO7NfJe4EXqcenqRZcvo",
    "https://drive.google.com/uc?export=view&id=1F53hVDMOXhdKuwtkJBQ1ovgRNBOwchcE"
  ]
  
  // Auto slide state for Japan 3 (Osaka) card
  const [currentJapan3ImageIndex, setCurrentJapan3ImageIndex] = useState(0)
  const japan3Images = [
    "https://drive.google.com/uc?export=view&id=1HiIpWCDN2K--GluZ03KxgaWkl-dccCRa",
    "https://drive.google.com/uc?export=view&id=1B5k-tXgBL8mrvIVqv_nK9e9goluppT4e",
    "https://drive.google.com/uc?export=view&id=1EJG6tH0nwDi8eO7NfJe4EXqcenqRZcvo",
    "https://drive.google.com/uc?export=view&id=1F53hVDMOXhdKuwtkJBQ1ovgRNBOwchcE"
  ]
  ...
  const [isFirstCardCompact, setIsFirstCardCompact] = useState(true)
  const [isSecondCardCompact, setIsSecondCardCompact] = useState(true)
  const [isThirdCardCompact, setIsThirdCardCompact] = useState(true)
  const [isFourthCardCompact, setIsFourthCardCompact] = useState(true)
  const [isFifthCardCompact, setIsFifthCardCompact] = useState(true)
  const [isSixthCardCompact, setIsSixthCardCompact] = useState(true)
  const [isSeventhCardCompact, setIsSeventhCardCompact] = useState(true)
  const [isEighthCardCompact, setIsEighthCardCompact] = useState(true)
  ...
  // Auto slide for Japan card
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentJapanImageIndex(prev => (prev + 1) % japanImages.length)
    }, 4000) // Change every 4 seconds
    
    return () => clearInterval(interval)
  }, [japanImages.length])

  // Auto slide for Japan 2 (Kyoto) card
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentJapan2ImageIndex(prev => (prev + 1) % japan2Images.length)
    }, 4200) // Change every 4.2 seconds (slightly different timing)
    
    return () => clearInterval(interval)
  }, [japan2Images.length])

  // Auto slide for Japan 3 (Osaka) card
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentJapan3ImageIndex(prev => (prev + 1) % japan3Images.length)
    }, 4400) // Change every 4.4 seconds (slightly different timing)
  ...
  const searchResults = useMemo(() => {
    ...
    const results = searchIndex.search(combinedFilters, sortOptions)
    
    // Filter out flash sale tours
    return results.filter(tour => {
      ...
      const isFlashSale = (Math.abs(hash) % 100) < 25
      return !isFlashSale // Exclude flash sale tours
    })
  ...
  const featuredTours = useMemo(() => {
    const tours = searchIndex.getFeaturedTours(12) // Get more initially
    // Filter out flash sale tours
    return tours.filter(tour => {
      ...
      const isFlashSale = (Math.abs(hash) % 100) < 25
      return !isFlashSale // Exclude flash sale tours
    }).slice(0, 6) // Take first 6 non-flash-sale tours
  }, [])
...
{/* External Card Footer - Days/Nights Info (Outside Banner like Agoda) */}
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองวันนี้ลดเพิ่ม 10%</span>
                    </div>
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ผ่อน 0% 4 เดือน</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองเร็วลด 40%</span>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        <div className="relative inline-block w-4 h-4">
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#E5E7EB"></path>
                          </svg>
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: 'inset(0px 0% 0px 0px)'}}>
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                          </svg>
                        </div>
                        <div className="relative inline-block w-4 h-4">
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20">
                        ...
                        <div className="relative inline-block w-4 h-4">
                          ...
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: 'inset(0px 20% 0px 0px)'}}>
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                          </svg>
                        </div>
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(7,842 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                      {country.name === 'เกาหลีใต้' ? (
                        <>
                          <span className="inline-block bg-gray-100 text-gray-700 px-2 py-1 rounded-md text-xs font-medium whitespace-nowrap flex-shrink-0">
                            โซล
                          </span>
                          <span className="inline-block bg-gray-100 text-gray-700 px-2 py-1 rounded-md text-xs font-medium whitespace-nowrap flex-shrink-0">
                            ปูซาน
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center justify-start gap-2 text-gray-600 mb-1">
                    <div className="flex items-center gap-1">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                      </svg>
                      <span className="text-xs">{country.name === 'เกาหลีใต้' ? '5 วัน' : country.name === 'ฮ่องกง' ? '4 วัน' : '5 วัน'}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-xs">{country.name === 'เกาหลีใต้' ? '4 คืน' : country.name === 'ฮ่องกง' ? '3 คืน' : '4 คืน'}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">{country.season}</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    </svg>
                    <span className="text-xs">ระดับโรงแรม</span>
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">{country.name === 'เกาหลีใต้' ? 'Korean Air (KE)' : country.name === 'ฮ่องกง' ? 'Cathay Pacific (CX)' : 'Thai Airways (TG)'}</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        {country.originalPrice && (
                          <div className="relative inline-block">
                            <span className="text-sm text-gray-600 font-medium">{country.originalPrice}</span>
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">{country.price}</div>
                      {country.originalPrice && (
                      ...
                      <div className="mt-1">
                        <button 
                          ...
                          className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group"
                        >
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
                        </button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="relative group bg-gradient-to-br from-pink-100 via-pink-50 to-rose-50 p-4 rounded-xl shadow-sm border border-pink-100 hover:shadow-md hover:border-pink-200 transition-all duration-300 transform hover:-translate-y-1">
                        <div className="absolute top-2 right-2 bg-black/60 text-white text-xs font-bold px-2 py-1 rounded">45821</div>
                      ...
                      <div className="relative group bg-gradient-to-br from-blue-100 via-blue-50 to-cyan-50 p-4 rounded-xl shadow-sm border border-blue-100 hover:shadow-md hover:border-blue-200 transition-all duration-300 transform hover:-translate-y-1">
                        <div className="absolute top-2 right-2 bg-black/60 text-white text-xs font-bold px-2 py-1 rounded">78234</div>
                      ...
                      <div className="relative group bg-gradient-to-br from-amber-100 via-amber-50 to-orange-50 p-4 rounded-xl shadow-sm border border-amber-100 hover:shadow-md hover:border-amber-200 transition-all duration-300 transform hover:-translate-y-1">
                        <div className="absolute top-2 right-2 bg-black/60 text-white text-xs font-bold px-2 py-1 rounded">92156</div>
                      ...
                      <div className="relative group bg-gradient-to-br from-emerald-100 via-emerald-50 to-green-50 p-4 rounded-xl shadow-sm border border-emerald-100 hover:shadow-md hover:border-emerald-200 transition-all duration-300 transform hover:-translate-y-1">
                        <div className="absolute top-2 right-2 bg-black/60 text-white text-xs font-bold px-2 py-1 rounded">31478</div>
      ...
      <main id="search-results" className={`max-w-7xl mx-auto px-4 py-8 ${showAdvancedModal ? 'hidden' : ''}`}>
        ...
        {isLoading ? (
          ...
          <div className="w-full py-8">
            ...
            {/* Grid for 10 Asian countries */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 gap-4 md:gap-6">
              
              {/* Japan */}
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  {/* Auto-sliding images */}
                  {japanImages.map((image, index) => (
                    <img 
                      ...
                      className={`w-full h-full object-cover group-hover:scale-110 transition-all duration-500 absolute inset-0 ${
                        index === currentJapanImageIndex ? 'opacity-100' : 'opacity-0'
                      }`}
                      loading="lazy"
                    />
                  ))}
                  ...
                  {/* Dot indicators */}
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-30" style={{gap: '4px', display: 'flex'}}>
                    {japanImages.map((_, index) => (
                      <button
                        key={`japan-dot-${index}`}
                        onClick={() => setCurrentJapanImageIndex(index)}
                        className={`rounded-full transition-all duration-200 ${
                          index === currentJapanImageIndex
                            ? 'bg-blue-600'
                            : 'bg-gray-300 hover:bg-gray-400'
                        }`}
                        style={{
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Japan Tokyo'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ญี่ปุ่น 5 วัน 4 คืน <span className="text-xl font-bold text-white">By TG Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>ชมซากุระบานสะพรั่ง ขึ้นโตเกียวทาวเวอร์ ช้อปปิ้งฮาราจูกุ-ชิบูย่า เที่ยวภูเขาไฟฟูจิชั้น 5 ล่องเรือทะเลสาบอาชิ แช่ออนเซนต้นตำรับ</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs">5 วัน</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-xs">4 คืน</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className="text-xs">ก.ย. 68 - มี.ค. 69</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Thai Airways (TG)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        <div className="relative inline-block">
                          <span className="text-sm text-gray-600 font-medium">฿52,300</span>
                          <div className="absolute left-0 w-full pointer-events-none" style={{top: '90%'}}>
                            <div 
                              className="h-0.5 bg-red-500 origin-left"
                              style={{
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿39,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
                        </a>
                      </div>
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  {/* Auto-sliding images */}
                  {japan2Images.map((image, index) => (
                    <img 
                      key={index}
                      src={image}
                      alt="ทัวร์ญี่ปุ่น เกียวโต เทศกาล"
                      className={`w-full h-full object-cover group-hover:scale-110 transition-all duration-500 absolute inset-0 ${
                        index === currentJapan2ImageIndex ? 'opacity-100' : 'opacity-0'
                      }`}
                      loading="lazy"
                    />
                  ))}
                  ...
                  {/* Dot indicators */}
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-30" style={{gap: '4px', display: 'flex'}}>
                    {japan2Images.map((_, index) => (
                      <button
                        key={`japan2-dot-${index}`}
                        onClick={() => setCurrentJapan2ImageIndex(index)}
                        className={`rounded-full transition-all duration-200 ${
                          index === currentJapan2ImageIndex
                            ? 'bg-blue-600'
                            : 'bg-gray-300 hover:bg-gray-400'
                        }`}
                        style={{
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Japan Kyoto'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ญี่ปุ่น 4 วัน 3 คืน <span className="text-xl font-bold text-white">By NH Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>วัดทองคำสีทองค่าตอนรุ่ง พิธีการชาไทย ชมคิมโนะยืนตอนกลางคืน สวมชุดไกตะสีสัน และถ่ายรูปทิวทัศน์กับป่าไผ่บามบู</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ทิป ไกด์ ครบวงจร</span>
                    </div>
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>โปรโมชันจำกัดเวลา</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ลิมิตเต็ดอิดิชั่น</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        ...
                        <div className="relative inline-block w-4 h-4">
                          ...
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: 'inset(0px 10% 0px 0px)'}}>
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                          </svg>
                        </div>
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(9,234 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs">4 วัน</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-xs">3 คืน</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className="text-xs">ต.ค. 68 - เม.ย. 69</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">ANA (NH)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        <div className="relative inline-block">
                          <span className="text-sm text-gray-600 font-medium">฿44,900</span>
                          <div className="absolute left-0 w-full pointer-events-none" style={{top: '90%'}}>
                            <div 
                              className="h-0.5 bg-red-500 origin-left"
                              style={{
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿32,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  {/* Auto-sliding images */}
                  {japan3Images.map((image, index) => (
                    <img 
                      key={index}
                      src={image}
                      alt="ทัวร์ญี่ปุ่น โอซาก้า ปราสาท"
                      className={`w-full h-full object-cover group-hover:scale-110 transition-all duration-500 absolute inset-0 ${
                        index === currentJapan3ImageIndex ? 'opacity-100' : 'opacity-0'
                      }`}
                      loading="lazy"
                    />
                  ))}
                  ...
                  {/* Dot indicators */}
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-30" style={{gap: '4px', display: 'flex'}}>
                    {japan3Images.map((_, index) => (
                      <button
                        key={`japan3-dot-${index}`}
                        onClick={() => setCurrentJapan3ImageIndex(index)}
                        className={`rounded-full transition-all duration-200 ${
                          index === currentJapan3ImageIndex
                            ? 'bg-blue-600'
                            : 'bg-gray-300 hover:bg-gray-400'
                        }`}
                        style={{
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Japan Osaka'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ญี่ปุ่น 4 วัน 3 คืน <span className="text-xl font-bold text-white">By JL Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>ปราสาทโอซาก้าใหญ่ที่สุดในแดนอินชีโอน เยี่ยมและลิ้มลองทาโกยากิแต่เดิม ชมวิวเมืองยามสีค๋า และเดินช้อปปิ้งที่มินามิเซะ</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ปราสาทโอซาก้า</span>
                    </div>
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ทาโกยากิแท้</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ช้อปปิ้งดอนกิโฮเต้</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        ...
                        <div className="relative inline-block w-4 h-4">
                          ...
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: 'inset(0px 30% 0px 0px)'}}>
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                          </svg>
                        </div>
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(4,567 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">พ.ย. 68 - พ.ค. 69</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">JAL (JL)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        <div className="relative inline-block">
                          <span className="text-sm text-gray-600 font-medium">฿37,900</span>
                          <div className="absolute left-0 w-full pointer-events-none" style={{top: '90%'}}>
                            <div 
                              className="h-0.5 bg-red-500 origin-left"
                              style={{
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿29,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Japan Hiroshima'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ญี่ปุ่น 4 วัน 3 คืน <span className="text-xl font-bold text-white">By TG Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>อนุสาวรีย์สันติภาพ และพิพิธภัณฑ์สันติภาพหิโรชิมา, เดินบนหอมทะเลทราย และขึ้นเกาะมิยาจิมา เพื่อไปจุดธูปสังเคราะห์</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>คืนเงิน 100% ถ้าฝนตก</span>
                    </div>
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ประกันการเดินทาง</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>รับเงินคืน 15%</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        ...
                        <div className="relative inline-block w-4 h-4">
                          ...
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: 'inset(0px 40% 0px 0px)'}}>
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                          </svg>
                        </div>
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(3,421 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">ก.พ. 68 - เม.ย. 68</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        <div className="relative inline-block">
                          <span className="text-sm text-gray-600 font-medium">฿34,900</span>
                          <div className="absolute left-0 w-full pointer-events-none" style={{top: '90%'}}>
                            <div 
                              className="h-0.5 bg-red-500 origin-left"
                              style={{
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿26,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          const hash = 'Japan Nara'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ญี่ปุ่น 4 วัน 3 คืน <span className="text-xl font-bold text-white">By NH Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>กวางนาราคู่ใจนักท่องเที่ยว เคียงข้างกับกวางไผ่เขียวแห่งคาซูก๋า เที่ยววัดโทไดจิอันโบราณ พร้อมสัมผัสกวางน้อยกวางใหญ่</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองวันนี้ลด 30%</span>
                    </div>
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองเร็วได้ของขวัญ</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ลิมิตเต็ดอิดิชั่น</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        ...
                        <div className="relative inline-block w-4 h-4">
                          ...
                          <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: 'inset(0px 50% 0px 0px)'}}>
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                          </svg>
                        </div>
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(5,789 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">มี.ค. 68 - พ.ค. 68</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">All Nippon Airways (NH)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        <div className="relative inline-block">
                          <span className="text-sm text-gray-600 font-medium">฿28,900</span>
                          <div className="absolute left-0 w-full pointer-events-none" style={{top: '90%'}}>
                            <div 
                              className="h-0.5 bg-red-500 origin-left"
                              style={{
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿22,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Hong Kong'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ฮ่องกง 4 วัน 3 คืน <span className="text-xl font-bold text-white">By CX Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>ดิสนีย์แลนด์สุดมวนเวทย์มนตร์ใจกลางเอเชีย ขึ้นเคเบิลคาร์วิคตอเรีย ถ่ายรูปคู่กับสกายบุรุษคลาสสิก และชมกลางคืนแสนสีสัน</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองเร็วลด 15%</span>
                    </div>
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ฟรีอัพเกรดห้อง</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองทันที ลด 15%</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">ธ.ค. 68 - มี.ค. 69</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Cathay Pacific (CX)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-xs text-gray-600 font-medium">เริ่มต้น</div>
                        <div className="relative inline-block">
                          <span className="text-sm text-gray-600 font-medium">฿59,900</span>
                          <div className="absolute left-0 w-full pointer-events-none" style={{top: '90%'}}>
                            <div 
                              className="h-0.5 bg-red-500 origin-left"
                              style={{
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'South Korea'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์เกาหลีใต้ 5 วัน 3 คืน <span className="text-xl font-bold text-white">By KE Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>K-Culture สุดเจ๋งที่โลตเต้เวิลด์ ชมหนังคอนเสิร์ต K-POP และพบไอดอลคู่ใจ เดินช้อปปิ้งที่แกงนัม ลิ้มลองคิมชิและบัลโกกี</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">22 ธ.ค. 67 - 26 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Korean Air (KE)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Taiwan'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ไต้หวัน 4 วัน 3 คืน <span className="text-xl font-bold text-white">By BR Airways</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>ตลาดกลางคืนไรผลไผวริกช้อปตลอด 24 ชม. ขึ้นดูตัวเมืองบนไทเป 101 เที่ยวชมบรรยากาศยามสวยและชิมขนมตาลทาไวย่</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, index) => (
                          <div key={index} className="relative inline-block w-4 h-4">
                            ...
                            <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: `inset(0px ${index < 4 ? '0%' : '40%'} 0px 0px)`}}>
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                            </svg>
                          </div>
                        ))}
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">20 ธ.ค. 67 - 23 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">EVA Air (BR)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Singapore'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์สิงคโปร์ 4 วัน 3 คืน <span className="text-xl font-bold text-white">By SQ Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>เมืองอนาคตแห่งอาเซียน ชมพิพิธภัณฑ์สวนน้ำ Gardens by the Bay เที่ยวสวนสัตว์ Universal Studios และชื่นชมมัรลายอน</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองด่วนเหลือ 3 ที่</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองเร็วลด 20%</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, index) => (
                          <div key={index} className="relative inline-block w-4 h-4">
                            ...
                            <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: `inset(0px ${index < 4 ? '0%' : '10%'} 0px 0px)`}}>
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                            </svg>
                          </div>
                        ))}
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">18 ธ.ค. 67 - 21 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Singapore Airlines (SQ)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Malaysia'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์มาเลเซีย 4 วัน 3 คืน <span className="text-xl font-bold text-white">By MH Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>ทาวเวอร์แฝดปีโทรนาส 88 ชั้นสูงสุด เที่ยวถ้ำบาตู และคาร์นิวอลกวางพีนัง ลิ้มลองอาหารหลายวัฒนธรรม</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, index) => (
                          <div key={index} className="relative inline-block w-4 h-4">
                            ...
                            <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: `inset(0px ${index < 4 ? '0%' : '50%'} 0px 0px)`}}>
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                            </svg>
                          </div>
                        ))}
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">15 ธ.ค. 67 - 18 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Malaysia Airlines (MH)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Indonesia'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์อินโดนีเซีย 5 วัน 4 คืน <span className="text-xl font-bold text-white">By GA Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>เกาะบาหลีที่รอคอยด้วยคลื่นสีฟ้าใส เที่ยววัดโบโรพุทโธที่โบราณที่สุด เที่ยวจอกใสเทลากาลูก และสปาทอง พร้อมชมนาซีกอริง</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, index) => (
                          <div key={index} className="relative inline-block w-4 h-4">
                            ...
                            <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: `inset(0px ${index < 4 ? '0%' : '30%'} 0px 0px)`}}>
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                            </svg>
                          </div>
                        ))}
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">12 ธ.ค. 67 - 16 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Garuda Indonesia (GA)</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end justify-start ml-4">
                      <div className="flex items-center gap-2 mb-1">
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Philippines'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ฟิลิปปินส์ 5 วัน 4 คืน <span className="text-xl font-bold text-white">By PR Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>เกาะโบราคาย์หาดทรายทรงสวยที่สุดในโลก ดำน้ำใสอ่าวปะการัง สนุกกับอาหารทะเลอร่อยและผ่อนคลายบนคลื่นสาว</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">10 ธ.ค. 67 - 14 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Philippine Airlines (PR)</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="flex items-center gap-2 justify-end mb-1">
                        <div className="text-right">
                          <div className="text-sm text-gray-500 font-medium relative">
                            ฿34,900
                            <div 
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Vietnam'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์เวียดนาม 5 วัน 4 คืน <span className="text-xl font-bold text-white">By VN Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>เรือโดยสารบนฮาลองเบย์ ชมหินทรายรูปทรงสีเขียวอมไอน้ำ เที่ยวโฮจิมินห์ไซตั๋วโลก ลิ้มลองกาแฟอิกและเบียร์เวียดนาม</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">8 ธ.ค. 67 - 12 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Vietnam Airlines (VN)</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="flex items-center gap-2 justify-end mb-1">
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Cambodia'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์กัมพูชา 4 วัน 3 คืน <span className="text-xl font-bold text-white">By K6 Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>นครวัดอังกอร์โบราณที่เก่าแก่ที่สุดในโลก เที่ยวเสียมเรียบสุดมหัศจรรย์ ชมหินปราสาทบายุน</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>จองด่วนเหลือ 2 ที่</span>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-2 py-1 rounded-lg text-xs font-bold">
                      <span>ผ่อน 0% 8 เดือน</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 mb-2">
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, index) => (
                          <div key={index} className="relative inline-block w-4 h-4">
                            ...
                            <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: `inset(0px ${index < 4 ? '0%' : '60%'} 0px 0px)`}}>
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                            </svg>
                          </div>
                        ))}
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(1,234 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">5 ธ.ค. 67 - 8 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Cambodia Airways (KR)</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="flex items-center gap-2 justify-end mb-1">
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿15,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
              ...
              <div className="group cursor-pointer">
                <div className="relative aspect-square overflow-hidden rounded-t-xl shadow-md hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="absolute top-0 right-0 z-20">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white px-4 py-1.5 rounded-bl-xl shadow-md">
                      <p className="text-base font-semibold tracking-wide">
                        TW{(() => {
                          // Generate 5-digit code based on country name hash
                          const hash = 'Laos'.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                          const code = String(10000 + (hash % 90000)).padStart(5, '0');
                          return code;
                        })()}
                      </p>
                  ...
                  <div className="absolute bottom-0 left-0 right-0">
                    <div className="text-white pt-2 pb-10 px-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent [text-shadow:_1px_1px_2px_rgba(0,0,0,0.5)]">
                      <h3 className="font-bold text-xl mb-1">ทัวร์ลาว 4 วัน 3 คืน <span className="text-xl font-bold text-white">By QV Airlines</span></h3>
                      <div className="text-base font-semibold text-white"><QuotedText><span>เมืองมรดกแห่งหลวงพระบาง ไปนั่งโป้งริมแม่น้ำโขง เที่ยววัดทองคำที่ประกายอย่างเนิบนิยม และเพลิดเพลินกับบรรยากาศเงียบสงบ</span></QuotedText></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-b-xl shadow-sm -mt-1">
                      ...
                      <div className="flex">
                        {[...Array(5)].map((_, index) => (
                          <div key={index} className="relative inline-block w-4 h-4">
                            ...
                            <svg className="absolute top-0 left-0 w-4 h-4" viewBox="0 0 20 20" style={{clipPath: `inset(0px ${index < 4 ? '0%' : '70%'} 0px 0px)`}}>
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" fill="#FBBF24"></path>
                            </svg>
                          </div>
                        ))}
                      </div>
                      <span className="text-xs text-gray-500 ml-1">(1,567 รีวิวจากผู้เดินทางจริง)</span>
                    </div>
                  </div>
                  <div className="mb-2 overflow-hidden">
                    <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-1" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
                  ...
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    ...
                    <span className="text-xs">3 ธ.ค. 67 - 6 ธ.ค. 67</span>
                  </div>
                  <div className="flex items-center gap-1 text-gray-600 mb-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                    ...
                    <div className="flex ml-1">
                      ...
                      <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Lao Airlines (QV)</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="flex items-center gap-2 justify-end mb-1">
                      ...
                      <div className="text-xl font-bold text-red-600 -mt-1">฿13,900</div>
                      <div className="mt-1">
                        <a href="#" className="text-blue-600 text-sm font-medium hover:text-blue-700 transition-colors inline-flex items-center group">
                          คลิกดูโปรโมชั่น
                          <span className="ml-1 inline-block animate-bounce-x">»</span>
