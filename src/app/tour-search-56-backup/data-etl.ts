...
class TourDataETL {
  ...
  private generateDefaultCities(country: string): string[] {
    // Generate multiple cities based on country
    const cityMap: Record<string, string[]> = {
      'ไต้หวัน': ['ไทเป', 'ไถจง', 'เกาสง'],
      'ญี่ปุ่น': ['โตเกียว', 'เกียวโต', 'โอซาก้า'],
      'เกาหลีใต้': ['โซล', 'ปูซาน', 'เชจู'],
      'จีน': ['ปักกิ่ง', 'เซี่ยงไฮ้', 'ซีอาน'],
      'สิงคโปร์': ['สิงคโปร์', 'เซนโตซา', 'มารีน่าเบย์'],
      'มาเลเซีย': ['กัวลาลัมเปอร์', 'ปีนัง', 'ลังกาวี'],
      'ฮ่องกง': ['ฮ่องกง', 'เกาลูน', 'แลนเทา'],
      'ยุโรป': ['ปารีส', 'โรม', 'ลอนดอน'],
      'ไทย': ['กรุงเทพฯ', 'เชียงใหม่', 'ภูเก็ต']
    }
    
    return cityMap[country] || [country, `${country}_2`, `${country}_3`]
  }
  ...
  public transformLegacyTour(
    ...
    const searchTour: SearchIndexTour = {
      ...
      location: {
        ...
        cities: legacyTour.destinations && legacyTour.destinations.length > 0 
          ? legacyTour.destinations 
          : this.generateDefaultCities(legacyTour.destination)
      },
