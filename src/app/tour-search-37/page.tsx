    ...
    <div className="ts32-container">
      ...
      <main id="search-results" className={`max-w-7xl mx-auto px-4 py-8 ${showAdvancedModal ? 'hidden' : ''}`}>
        ...
        {isLoading ? (
          ...
          >
            {/* Improved Tour Card - แสดงก่อน tours ปกติ */}
            {searchResults.length > 0 && (
              <div className="col-span-full mb-6">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-2xl border-2 border-blue-200 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-blue-700 font-semibold text-sm">แนะนำพิเศษ - ดีไซน์ใหม่</span>
                  </div>
                  <ImprovedTourCard
                    tour={searchResults[0]}
                    showTravelDates={showImprovedCardDates}
                    onToggleDates={() => setShowImprovedCardDates(!showImprovedCardDates)}
                    onBook={() => {
            ...
            {/* Duplicate Cards - เพิ่มอีก 2 ใบ */}
            {searchResults.length > 1 && (
              <div className="col-span-full grid md:grid-cols-2 gap-6 mb-6">
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-2xl border-2 border-green-200 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="bg-green-500 text-white p-1.5 rounded-lg">
                      <Sparkles className="w-4 h-4" />
                    </div>
                    <span className="text-green-700 font-semibold text-sm">ยอดนิยม #1</span>
                  </div>
                  <ImprovedTourCard
                    tour={searchResults[1]}
                    showTravelDates={false}
                    onToggleDates={() => {}}
                    onBook={() => {
                      setSelectedTourForBooking(searchResults[1])
                      setShowBookingModal(true)
                    }}
                  />
                </div>
                ...
                {searchResults.length > 2 && (
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-2xl border-2 border-purple-200 shadow-sm">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-purple-700 font-semibold text-sm">กำลังฮิต</span>
                    </div>
                    <ImprovedTourCard
                      tour={searchResults[2]}
                      showTravelDates={false}
                      onToggleDates={() => {}}
                      onBook={() => {
                        setSelectedTourForBooking(searchResults[2])
                        setShowBookingModal(true)
                      }}
                    />
                  </div>
            ...
            {/* Compact Tour Cards Section - New Style */}
            {searchResults.length > 3 && (
              <div className="col-span-full mb-6">
                <div className="mb-4">
                  <h3 className="text-lg font-bold text-gray-900 mb-2">ทัวร์แบบ Compact</h3>
                  <p className="text-sm text-gray-600">สไตล์การแสดงผลแบบใหม่ที่ประหยัดพื้นที่</p>
                </div>
                
                <div className="space-y-4">
                  {/* Minimal Variant */}
                  <CompactTourCard
                    tour={searchResults[3]}
                    variant="minimal"
                    onClick={() => {}}
                    onBookClick={() => {
                      setSelectedTourForBooking(searchResults[3])
                      setShowBookingModal(true)
                    }}
                  />
                  ...
                  {searchResults.length > 4 && (
                    <CompactTourCard
                      tour={searchResults[4]}
                      variant="feature"
                      onClick={() => {}}
                      onBookClick={() => {
                        setSelectedTourForBooking(searchResults[4])
                        setShowBookingModal(true)
                      }}
                    />
                  )}
                  ...
                  {searchResults.length > 5 && (
                    <CompactTourCard
                      tour={searchResults[5]}
                      variant="premium"
                      onClick={() => {}}
                      onBookClick={() => {
                        setSelectedTourForBooking(searchResults[5])
                        setShowBookingModal(true)
                      }}
                    />
                  )}
            ...
            {searchResults.map((tour, index) => (
              <TourCardComponent
                ...
                isFirstCard={index <= 7}
                isCompact={
                  index === 0 ? isFirstCardCompact : 
                  index === 1 ? isSecondCardCompact : 
                  index === 2 ? isThirdCardCompact :
                  index === 3 ? isFourthCardCompact :
                  index === 4 ? isFifthCardCompact :
                  index === 5 ? isSixthCardCompact :
                  index === 6 ? isSeventhCardCompact :
                  index === 7 ? isEighthCardCompact :
                  false
                }
                compactVersion={
                  index === 0 ? 'v11' : 
                ...
                onExpandToggle={
                  index === 0 ? handleFirstCardExpandToggle : 
                  index === 1 ? handleSecondCardExpandToggle :
                  index === 2 ? handleThirdCardExpandToggle :
                  index === 3 ? handleFourthCardExpandToggle :
                  index === 4 ? handleFifthCardExpandToggle :
                  index === 5 ? handleSixthCardExpandToggle :
                  index === 6 ? handleSeventhCardExpandToggle :
                  index === 7 ? handleEighthCardExpandToggle :
                  undefined
                }
