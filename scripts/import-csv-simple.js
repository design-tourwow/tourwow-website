...
async function importCSV() {
  try {
    ...
    // Import TW Page Types first
    await importTwPageTypes();
    
    // Import a sample of Product Pool (first 100 records)
    await importProductPoolSample();
    
    // Import Agency Blogs with safe date parsing
    await importAgencyBlogsSafe();
...
async function importProductPoolSample() {
  console.log('📦 Importing Product Pool Sample (first 100 records)...');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(filePath)
      .pipe(csv())
      .on('data', (data) => {
        if (count >= 100) return; // Only process first 100 records
...
async function importAgencyBlogsSafe() {
  console.log('📝 Importing Agency Blogs (first 50 records)...');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(filePath)
      .pipe(csv())
      .on('data', (data) => {
        if (count >= 50) return; // Only process first 50 records
